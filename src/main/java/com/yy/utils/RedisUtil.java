...
public class RedisUtil {
	 ...
	 * @param list
	 */
	public String setList(String key ,int indexdb,List<?> list){
		Jedis jedis = null;
		String value = null;
		try {
			jedis = jedisPool.getResource();
	 ...
	 * @param clazz
	 */
	public <T> List<T> getList(String key ,int indexdb,Class<T> clazz){
		Jedis jedis = null;
		...
		try {
			...
			if (StringUtils.isNotBlank(jsonArray)){
				list = JSONObject.parseArray(jsonArray, clazz);
			}
		} catch (Exception e) {
			log.error(e.getMessage());
		} finally {
	 ...
	 * @return 成功返回1 如果存在 和 发生异常 返回 0
	 */
	public Long expire(String key,long value,int indexdb) {
		Jedis jedis = null;
		try {
			jedis = jedisPool.getResource();
			jedis.select(indexdb);
	...
	/**
	 * <p>
	 * 通过key 对value进行加值+1操作,当value不是int类型时会返回错误,当key不存在是则value为1,指定数据库
	 * </p>
	 *
	 * @param key
	 * @return 加值后的结果
	 */
	public Long incr(String key,Integer indexDb) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = jedisPool.getResource();
	...
	/**
	 * <p>
	 * 通过key给指定的value加值,如果key不存在,则这是value为该值,定位数据库
	 * </p>
	 *
	 * @param key
	 * @param integer
	 ...
	 * @return 如果存在返回0 异常返回null
	 */
	public Long hsetEx(String key, String field, String value, int indexdb, int expireSeconds) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = jedisPool.getResource();
	 ...
	 * @return 没有返回null
	 */
	public String hget(String key, int indexdb, String field) {
		Jedis jedis = null;
		String res = null;
		try {
			jedis = jedisPool.getResource();
